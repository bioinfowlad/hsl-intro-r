...
![Default RStudio Interface](RStudio-Screenshot.png)
...
R is full of more sophisticated functions

*`chunk 5`*
```{r}
linreg <- lm(y~x) #linreg is now an object containing the model and info of the fit
summary(linreg)
```
...
Example 2: How do I do regression in R?

*`chunk 7`*
```{r}
??regression
# The double question mark is really shorthand for the help.search command:
help.search('cohen kappa')
```
...
sample_size <- data.frame(straindata$sample, straindata$genome_size)
...
### Disgression: the `apply` function

When dealing with large tables of data, even vectorization may not be enough to elegantly transform data. The apply function lets us apply functions over multiple rows or columns of a data table at once.

*`chunk 26`*
```{r}
apply(inflammation[2:6,3:32],MARGIN=1,FUN=mean)
# Mean inflamation of first five patients over all days
apply(inflammation[,3:5],MARGIN=2,FUN=mean)
# Mean inflammation of all patients on first three days
```
...
- Videos from rstudio::conf <https://www.rstudio.com/resources/videos/rstudioconf-2018-talks/>
